...
if (isset($_POST['submit_offer'])) {
    ...
    $insert_offer = $mysqli->prepare("INSERT INTO `offers` (ID_Article, ID_Buyer, offer_price, status) VALUES (?, ?, ?, 'Pending')");
    if (!$insert_offer) {
        die('Error: ' . $mysqli->error);
    }
    $insert_offer->bind_param("iid", $ID_Article, $_SESSION["user_id"], $offer_price);
    $insert_offer_result = $insert_offer->execute();

    if ($insert_offer_result) {
        $message = 'Your offer has been submitted!';
    } else {
        $message = 'Error submitting your offer: ' . $mysqli->error;
    }
}
    ...
    <form action="" method="post" class="box">
        ...
        <div class="row">
            ...
            <div class="content">
                ...
                <div class="flex">
                    ...
                    <?php elseif ($product['selling_type'] === 'Best Offer'): ?>
                        <div class="best-offer-inputs">
                            ...
                            <input type="number" name="offer_price" id="offer_price" min="<?= $product['price']; ?>"
                                   step="0.01" required>
                        </div>
                    <?php else: ?>
                ...
                <?php elseif ($product['selling_type'] === 'Best Offer'): ?>
                    <!-- Display Best Offer functionality -->
                    <form action="submit_offer.php" method="post">
                        <input type="hidden" name="item_id" value="<?= $product['ID_Article']; ?>">
                        <input type="hidden" name="seller_id" value="<?= $sellerId; ?>">
                        <input type="submit" value="Submit Offer" class="btn" name="sendoffer">
                    </form>
                <?php else: ?>
                    <div class="flex-btn">
